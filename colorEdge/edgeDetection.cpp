...
int save_RGB(IMAGE r, IMAGE g, IMAGE b, const char* name)
{
	Mat image;
	int i, j, k;
...
/*		for (i=0; i<r->info->nr; i++)
	...
	imshow("save_rgb", image);
	return 1 - k;
}


int hue(unsigned char R, unsigned char G, unsigned char B)
{
	int cMin, cMax, c;
	int H, Rdelta, Gdelta, Bdelta;

	if (R >= G && R >= B) cMax = R;
	else if (G >= R && G >= B) cMax = G;
	else cMax = B;
	...
	{
		Rdelta = (((cMax - R) * (255 / 6)) + ((cMax - cMin) / 2)) / (cMax - cMin);
		Gdelta = (((cMax - G) * (255 / 6)) + ((cMax - cMin) / 2)) / (cMax - cMin);
		Bdelta = (((cMax - B) * (255 / 6)) + ((cMax - cMin) / 2)) / (cMax - cMin);

		if (R == cMax)
			H = Bdelta - Gdelta;
		else if (G == cMax)
			H = (255 / 3) + Rdelta - Bdelta;
		else /* B == cMax */
			H = ((2 * 255) / 3) + Gdelta - Rdelta;
